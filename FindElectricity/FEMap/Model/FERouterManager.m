...
//  FindElectricity
//
//  Created by DongQiangLi on 2017/11/2.
//  Copyright © 2017 LiDongQiang. All rights reserved.
//
...
/**
 ...
 - DQRouteNaviTypeApple: 苹果地图导航
 - DQRouteNaviTypeGaode: 高德地图导航
 - DQRouteNaviTypeBaidu: 百度地图导航
 */
typedef NS_ENUM(NSUInteger, DQRouteNaviType) {
    DQRouteNaviTypeApple,
    DQRouteNaviTypeGaode,
    DQRouteNaviTypeBaidu,
};

static NSString *_defaultDestinationName = @"目的地";

@implementation FERouterManager
...
+ (void)p_presentRouteNaviMenuOnController:(UIViewController *)controller withLocation:(nullable CLLocation *)location destination:(nullable NSString *)destination {
    ...
    //========== 使用苹果地图导航 ==========//
    if (canOpenAppleMap) {
        [alertVC addAction:[self p_actionWithNaviType:DQRouteNaviTypeApple title:@"使用苹果自带地图导航" location:location destination:destination]];
    }
    
    //========== 使用高德地图导航 ==========//
    if (canOpenGaodeMap) {
        [alertVC addAction:[self p_actionWithNaviType:DQRouteNaviTypeGaode title:@"使用高德地图导航" location:location destination:destination]];
    }
    
    //========== 使用百度地图导航 ==========//
    if (canOpenBaiduMap) {
        [alertVC addAction:[self p_actionWithNaviType:DQRouteNaviTypeBaidu title:@"使用百度地图导航" location:location destination:destination]];
    }
...
+ (UIAlertAction *)p_actionWithNaviType:(DQRouteNaviType)naviType title:(NSString *)title location:(CLLocation *)location destination:(NSString *)destination {
    // 目的地如果为空，展示名称默认为”目的地“
    ...
    UIAlertAction *action = [UIAlertAction actionWithTitle:title style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        switch (naviType) {
            case DQRouteNaviTypeApple: // 苹果地图
            {
                if (location) {
            ...
            case DQRouteNaviTypeGaode: // 高德地图
            {
                if (location) {
                    // 有坐标时以坐标为准
            ...
            case DQRouteNaviTypeBaidu: // 百度地图
            {
                if (location) {
                    // 注：高德用的gcj02坐标系
