...
+ (MANaviRoute *)naviRouteForRailway:(AMapRailway *)railway
{
    ...
    for (int i = 0; i < stations.count - 1; i++)
    {
        ...
        naviPolyline.type = FEPointAnnotRailway;
        ...
        annotation.type = FEPointAnnotRailway;
...
+ (MANaviRoute *)naviRouteForTaxi:(AMapTaxi *)taxi
{
    ...
    MAPolyline *polyline = [MAPolyline polylineWithCoordinates:coordinates count:2];
    MANaviPolyline *naviPolyline = [[MANaviPolyline alloc] initWithPolyline:polyline];
    naviPolyline.type = FEPointAnnotBus;
...
+ (MANaviRoute *)naviRouteForWalking:(AMapWalking *)walking
{
    ...
    [walking.steps enumerateObjectsUsingBlock:^(AMapStep *step, NSUInteger idx, BOOL *stop) {
        ...
        if (stepPolyline != nil)
        {
            ...
            naviPolyline.type = FEPointAnnotWalking;
            
            [polylines addObject:naviPolyline];
            
            FEPointAnnotation * annotation = [[FEPointAnnotation alloc] init];
            annotation.coordinate = MACoordinateForMapPoint(stepPolyline.points[0]);
            annotation.type = FEPointAnnotWalking;
...
+ (MANaviRoute *)naviRouteForSegment:(AMapSegment *)segment
{
    ...
    if (busLinePolyline)
    {
        ...
        FEPointAnnotation * bus = [[FEPointAnnotation alloc] init];
        bus.coordinate = MACoordinateForMapPoint(busLinePolyline.points[0]);
        bus.type = FEPointAnnotBus;
...
+ (instancetype)naviRouteForPath:(AMapPath *)path withNaviType:(FEPointAnnotType)type showTraffic:(BOOL)showTraffic startPoint:(AMapGeoPoint *)start endPoint:(AMapGeoPoint *)end
{
...
- (instancetype)initWithPath:(AMapPath *)path withNaviType:(FEPointAnnotType)type showTraffic:(BOOL)showTraffic startPoint:(AMapGeoPoint *)start endPoint:(AMapGeoPoint *)end
{
    self = [self init];
    ...
    // 为drive类型且需要显示路况
    if (showTraffic && type == FEPointAnnotDrive)
    {
