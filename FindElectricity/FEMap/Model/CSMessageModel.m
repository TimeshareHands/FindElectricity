...
- (CGRect)messageFrame
{
    ...
    CGFloat maxWith = ScreenWidth * 0.7 - 60;
     CGSize size = [self labelAutoCalculateRectWith:self.messageText Font:[UIFont fontWithName:MTBaseFont size:16] MaxSize:CGSizeMake(maxWith, MAXFLOAT)];
...
- (CGRect)voiceFrame
{
    ...
    if (self.messageSenderType == MessageSenderTypeMe)
    {
        rect = CGRectMake(SCREEN_WIDTH * 0.3 + 10 + maxWith - maxWith *(self.duringTime/20.0 > 1? 1 :self.duringTime/20.0), timeLabelHeight + 10, maxWith *(self.duringTime/20.0 > 1? 1 :self.duringTime/20.0), 44);
    }
    else
    {
