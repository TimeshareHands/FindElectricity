...
@implementation MTResourceManager
...
- (CAGradientLayer *)gradientLayerWithColors:(NSArray *)colors locations:(NSArray *)locations starPoint:(CGPoint)startPiont endPoint:(CGPoint)endPoint superView:(UIView *)superView
{
    CAGradientLayer *colorLayer = [CAGradientLayer layer];
    //    colorLayer.frame    = (CGRect){CGPointZero, CGSizeMake(kWidth-kLeftOrRightWidth-16, 60)};
    colorLayer.frame = superView.bounds;
    //    colorLayer.frame = CGRectMake(0, 0, kWidth-kLeftOrRightWidth-25, superView.height);
    ...
    // 起始点
    colorLayer.startPoint = startPiont;
...
+ (CGSize)heightWithString:(NSString *)string width:(CGFloat)width font:(UIFont *)font
{
    ...
    //
    //    CGSize size = [string boundingRectWithSize:CGSizeMake(width, CGFLOAT_MAX) options:(NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading) attributes:attributes context:nil].size;
    
    CGSize size = [string  boundingRectWithSize:CGSizeMake(width, MAXFLOAT) options:(NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading) attributes:@{NSFontAttributeName:font} context:nil].size;
    return size;
}
